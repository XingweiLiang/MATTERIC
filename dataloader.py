...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, path, roberta_path, train=True):
...
class MELDDataset(Dataset):
    ...
    def __init__(self, path, roberta_path, n_classes, train=True):
        if n_classes == 3:
...
def get_loaders(feature_path, dataset='IEMOCAP', batch_size=32, valid=0.1, num_workers=0, pin_memory=False, n_classes=3):
    if dataset == 'IEMOCAP':
        ...
        roberta_path = feature_path + 'IEMOCAP_features/IEMOCAP_features_roberta.pkl'
        trainset = IEMOCAPDataset(path=path, roberta_path=roberta_path)
        ...
        roberta_path = feature_path + 'MELD_features/MELD_features_roberta.pkl'
        trainset = MELDDataset(path=path,  roberta_path=roberta_path, n_classes=n_classes)
        testset = MELDDataset(path=path,  roberta_path=roberta_path, n_classes=n_classes, train=False)
